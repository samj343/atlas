...
class MarketMaker:
    ...
    def _make_quote(self, option: BinaryOption) -> Quote:
        ...
        half_spread = self.SPREAD_BASE + self.SPREAD_CURVE * fair * (1.0 - fair)
        ...
        # Adaptive sizing: grow with the bankroll, floor at v2's proven size,
        # and never let one side commit more than the remaining cash.
        def side_quantity(loss_per_contract: float) -> int:
            if loss_per_contract <= 0.004:
                return self.QUOTE_SIZE_MAX
            scaled = int(self.SIZE_FRACTION * cash / loss_per_contract)
            quantity = min(self.QUOTE_SIZE_MAX, max(self.QUOTE_SIZE_MIN, scaled))
            if loss_per_contract * quantity > cash:
                quantity = int(cash / loss_per_contract)
            return quantity

        bid_quantity = side_quantity(bid)
        if bid_quantity < 1:
            bid, bid_quantity = 0.0, 1
        offer_quantity = side_quantity(1.0 - offer)
        if offer_quantity < 1:
